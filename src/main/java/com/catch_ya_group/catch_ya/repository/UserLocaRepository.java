...
public interface UserLocaRepository extends JpaRepository<UserLoca, Long> {
    ...
    @Query(value = """
        SELECT l.user_id as userId,
               ...
               case when lb.viewed_cnt is null then 0
               else lb.viewed_cnt end viewedCnt,
               s.day_status as instanceContent,
               s.status_date as instancePostDate
        FROM user_loca l
    ...
    LEFT JOIN leaderboard lb ON lb.leaderboard_id = u.user_id
    """, nativeQuery = true)
    List<UserLocaResponseProjection> getAllUsersLoca();
